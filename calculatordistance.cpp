...
int CalculatorDistance::getPosition(int* rssi, double *x, double *y)
{
    //it is assumed to receive 0 as last parameters
    int n_ESP=0, i, j, c, ESP_used[4];
    double d[4], q[4][4], qf[4];
    // d is a vector which contains the distances from the ESPs
    for(i=0; i<4; i++){
        ESP_used[i]=0;
        qDebug() << "RSSI " << i << ": " << rssi[i];
    }
    for (i = 0; i < 4; i++)
    {
        if(rssi[i] != 0)
        {
            d[i] = RSSItoDinstance(rssi[i]);
            ESP_used[i] = 1;
            n_ESP++;
        }
    }
    ...
    for (i = 0; i < 4; i++)
    {
        if(ESP_used[i]==1)
        {
            //cout << "xyESP " << (double) ESP_position[i][0] << " " << (double) ESP_position[i][1] << "\n";
            q[i][0] = (double) ESP_position[i][0] - d[i];
            //cout << "q[i][0] " << q[i][0] << "\n";
            q[i][1] = (double) ESP_position[i][1] - d[i];
            //cout << "q[i][1] " << q[i][1] << "\n";
...
void CalculatorDistance::setBoardsPosition(int index, int x, int y, QString mac){
    ...
    if(x == 0 && y == 0)
        ...
        boardMap.insert(mac, index);
    //qDebug() << boardMap.lastKey();
}
...
int CalculatorDistance::getBoardVec(QString mac){
    int index = 0;
//    QMap<QString, int>::const_iterator i = boardMap.constBegin();
//    while(i != boardMap.constEnd()){
//        if(QString::compare(i.key(), mac) == 0){
//            qDebug() << "CONFRONTO!";
//            index = i.value();
//            break;
//        }
//    }
    ...
    for(auto e : boardMap.toStdMap()){
      //  qDebug() << "KEY: " << e.first << ", VALUE: " << e.second;
      //fout << e.first << "," << e.second << '\n';
        if(QString::compare(e.first, mac) == 0){
            //qDebug() << "CONFRONTO!";
            index = e.second;
            break;
        }
